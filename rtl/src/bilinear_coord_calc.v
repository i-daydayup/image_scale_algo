...
module bilinear_coord_calc #(
	...
	input  wire [15:0]                   dst_idx,      // 目标像素索引
	input  wire [INT_BITS+FRAC_BITS-1:0] inv_scale,    // 逆缩放比例 (Qm.n)
	input  wire [15:0]                   src_size,     // 源图像尺寸

	output reg  [15:0]                   src_pos_int,  // 源图像素整数坐标
	output reg  [WEIGHT_BITS-1:0]        src_pos_frac, // 源图像素小数部分 (Q0.w)
	...
	always @(posedge clk or negedge rst_n) begin
		if (rst_n == 1'b0) begin
			...
			mult_a      <= {dst_idx, 1'b1} + 1;  // 2*dst_idx + 1
			mult_b      <= inv_scale;
			src_size_d1 <= src_size;
			valid_d1    <= 1;
		end
	end
	...
	always @(posedge clk or negedge rst_n) begin
		if (rst_n == 1'b0) begin
			...
			// mult_result >> 1 - 0.5
			src_pos_fixed <= (mult_result >> 1) - HALF_N;
	...
	always @(posedge clk or negedge rst_n) begin
		if (rst_n == 1'b0) begin
			...
			// 整数部分：右移 FRAC_BITS 位
			src_pos_int <= src_pos_clamped >> FRAC_BITS;
