...
module bilinear_scaler #(
	...
	input  wire [INT_BITS+FRAC_BITS-1:0] cfg_inv_scale_x, // X方向逆缩放比例定点值
	input  wire [INT_BITS+FRAC_BITS-1:0] cfg_inv_scale_y, // Y方向逆缩放比例定点值
	input  wire [15:0]                   cfg_dst_width,   // 目标图像宽度
	input  wire [15:0]                   cfg_dst_height,  // 目标图像高度
	input  wire [15:0]                   cfg_src_width,   // 源图像宽度
	input  wire [15:0]                   cfg_src_height,  // 源图像高度

	// 输入视频流 (AXI-Stream 风格)
	input  wire                          i_valid,
	input  wire [DATA_WIDTH-1:0]         i_data,
	input  wire                          i_last,        // 行结束标记
	input  wire                          i_frame_start, // 帧开始标记 (tuser)
	output wire                          i_ready,

	// 输出视频流
	output reg                           o_valid,
	output reg  [DATA_WIDTH-1:0]         o_data,
	output reg                           o_last,        // 行结束标记
	output reg                           o_frame_start, // 帧开始标记
	input  wire                          o_ready
);
	...
	always @(posedge clk or negedge rst_n) begin
		if (rst_n == 1'b0) begin
			...
			r_inv_scale_x  <= cfg_inv_scale_x;
			r_inv_scale_y  <= cfg_inv_scale_y;
			r_dst_width    <= cfg_dst_width;
			r_dst_height   <= cfg_dst_height;
			r_src_width    <= cfg_src_width;
			r_src_height   <= cfg_src_height;
		end
	end
	...
	always @(posedge clk or negedge rst_n) begin
		...
		else if (state == READ_LINE && i_valid) begin
			if (i_last == 1'b1) begin
				src_x_cnt <= 0;
				src_y_cnt <= src_y_cnt + 1;
			end
			else begin
				src_x_cnt <= src_x_cnt + 1;
			end
		end
	end
	...
	always @(posedge clk) begin
		if (lb_wen == 1'b1) begin
			if (lb_swap == 1'b1) begin
				line_buffer_1[lb_waddr] <= lb_wdata;
			end
			else begin
				line_buffer_0[lb_waddr] <= lb_wdata;
			end
		end
	end
	...
	always @(posedge clk or negedge rst_n) begin
		if (rst_n == 1'b0) begin
			...
			case (pixel_fetch_state)
				FETCH_IDLE: begin
					pixel_fetch_state <= FETCH_P00;
					lb_raddr <= coord_x_int;
				end
				FETCH_P00: begin
					// 第2拍：读取P00 (y0行, x0列)
					p00 <= lb_rdata0;
					pixel_fetch_state <= FETCH_P10;
					lb_raddr <= coord_x_int + 1;
				end
				FETCH_P10: begin
					// 第3拍：读取P10 (y0行, x1列)
					p10 <= lb_rdata0;
					p01 <= lb_rdata1;  // 同时读取P01
					pixel_fetch_state <= FETCH_P11;
					lb_raddr <= coord_x_int + 1;
				end
				FETCH_P11: begin
					// 第4拍：读取P11 (y1行, x1列)
					p11 <= lb_rdata1;
					pixel_fetch_state <= FETCH_DONE;
				end
				FETCH_DONE: begin
					pixel_fetch_state <= FETCH_IDLE;
				end
	...
	always @(posedge clk) pixel_fetch_done_d1 <= (pixel_fetch_state == FETCH_DONE);
	...
	always @(posedge clk or negedge rst_n) begin
		if (rst_n == 1'b0) begin
			...
			o_valid       <= interp_valid && (state == PROCESS);
			o_data        <= interp_result;
			o_last        <= interp_valid && (dst_x_cnt >= r_dst_width - 1);
			o_frame_start <= (state == PROCESS) && (dst_y_cnt == 0) && (dst_x_cnt == 0) && pixel_fetch_done_d1;
		end
	end
