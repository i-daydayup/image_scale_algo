...
module bilinear_scaler_vh #(
	...
	//========================================================================
	// 4-tap移位寄存器（双线性实际用2点，保留4-tap架构）
	reg [DATA_WIDTH-1:0] h_shift_reg [0:1];  // 双线性只需2个
	...
	always @(posedge clk_out) begin
		if (v_pix_valid == 1'b1) begin
			h_shift_reg[1] <= #U_DLY h_shift_reg[0];
			h_shift_reg[0] <= #U_DLY v_pix_result;
		end
	end
	
	// 双线性水平插值（简化：直接对当前两个点插值）
	always @(posedge clk_out) begin
		h_pix_result <= #U_DLY (h_shift_reg[0] * w_x0 + h_shift_reg[1] * w_x1) >> FRAC_BITS;
